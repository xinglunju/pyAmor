...
def __xcorrelate__(spec, vaxis):
	...
	kernel = np.zeros(len(vaxis))
	sigmav = 1.0
	for i in np.arange(len(voff_lines)):
		kernel += np.exp(-(vaxis-vaxis[len(vaxis)/2]-voff_lines[i])**2/(2*sigmav**2))*tau_wts[i]
	lags = np.correlate(spec,kernel,'same')
	vlsr = vaxis[lags.argmax()]
	return vlsr
...
def fit_2comp(data_low, data_upp, ra_range=[0,1], dec_range=[0,1], cutoff=0.010):
	"""
	...
	for i in dec_range:
		for j in ra_range:
			...
			if vlsr1 != 0:
				...
				params.add('peakv', value=vlsr1, min=vlsr1-2*onevpix, max=vlsr1+2*onevpix)
			...
			if vlsr2 != 0:
				...
				params.add('peakv_c2', value=vlsr2, min=vlsr2-2*onevpix, max=vlsr2+2*onevpix)
			...
			#tauupp[i,j] = params['tau22'].value
			#print 'finish pixel', i, j
	...
	fits.writeto('linew_upp.fits', linew22, header=hdrt, clobber=True)
	hdrt['bunit'] = '1'
...
##fits.writeto('NH3_22_comb.cm.regrid.fits',data2,hdr1,clobber=True)

firstguess, xaxis_gbt, yaxis_gbt = __loadvlsr__('result_test.txt','NH3_11_rmbl.fits')

fit_2comp(data1,data2,ra_range=np.arange(naxisx),dec_range=np.arange(naxisy),cutoff=0.009)
...
print 'Center of the image is', x_c, y_c
fig.recenter(x=x_c, y=y_c, radius=0.05)
fig.show_colorscale(vmin=0,vmax=5)
fig.add_colorbar()
fig.save('lw_low.png')
